...
請嚴格檢視 AI 戰略軍師提出的「Top 50 戰略提案」。

=== 我們的核心研究計畫（完整版 — 你必須讀完再審查）===
{research_context}

你的審查核心是基於上述《研究說明書》定義的 130 個問題（H01-H23、M01-M22、R01-R18、F01-F18、Q01-Q19、E01-E15、B01-B15）以及 P1-P4 四大根本困境，並且以實務軟體工程師的角度戳破學術界的幻想。
...
2. 6M1E 全維度實務審查（不能只看 Brownfield）：提案是否忽略了我們工廠的 130 個致命問題？請結合具體問題編號駁火：
   - 人(H)：師傅經驗消失(H01)、口傳培訓(H02)、決策靠感覺(H07)、班別交接漏訊(H11)
   - 機(M)：老折床無網路(M01)、PLC信號封閉(M02)、機台稼動率未知(M06)
   - 料(R)：材證紙本(R01)、彈回量批次差異(R06)、板厚實測消失(R05)
   - 法(F)：NC程式版本失控(F01)、DXF版本不對應(F02)、SOP靠記憶(F10)
   - 測(Q)：角度量測紙本(Q01)、補償值消失(Q02)、首件FAI歸檔消失(Q03)
   - 環(E)：金屬反光(E01)、光源不穩(E02)、油膜干擾(E04)
   - Brownfield(B)：安全迴路禁改(B01)、冷啟動零資料(B06)、誤報失信(B13)
3. 實務生產節奏衝擊：系統要求是否會打亂師傅的步調 (B05)？端到端延遲是否會嚴重拖慢製程 (B12)？系統犯錯時會不會亂攔截導致師傅暴怒不用 (B13)？
4. 與 RBv3 事件化證據鏈的相容性：該提案是否與我們的「三源共識」、「降級矩陣」、「PFID 設計指紋」等方法論相容？
...
### 審查 TOP 1: [文章標題]
- **【判定結果】**: PASS 或 REJECT
- **【廠長嚴批】**: (詳細說明理由，必須引用上述研究說明書中的具體問題編號，如 H01、M01、B06 等。不得使用空泛批評。)
